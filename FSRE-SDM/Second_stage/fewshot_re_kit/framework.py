...
class FewShotREFramework:
    ...
    def train(self,
        ...
        component1 = ['GAT_feature', 'GAT_label','Input_example','Input_rel','MLP',"Poto_layer"]
        component2 = ["GAT_label"]
        parameters = [
        ...
        parameters_G = [
            {'params': [p for n, p in parameters_to_optimize
                        if not any(nd in n for nd in no_decay) and any(nd in n for nd in component2)],
             'weight_decay': weight_decay,
             'lr': learning_rate * 2
             },
            {'params': [p for n, p in parameters_to_optimize
                        if any(nd in n for nd in no_decay) and any(nd in n for nd in component2)],
             'weight_decay': 0.0,
             'lr': learning_rate * 2
             }
        ]
        ...
        if use_sgd_for_bert:
            ...
            optimizer = AdamW(parameters_G, lr=learning_rate, correct_bias=False)
            # optimizer_G = AdamW(parameters_G, lr=learning_rate, correct_bias=False)
            optimizer_G = torch.optim.Adam(model.Poto_layer.parameters(), lr=learning_rate)

        scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=warmup_step, num_training_steps=train_iter)
        scheduler_G = get_linear_schedule_with_warmup(optimizer_G, num_warmup_steps=warmup_step, num_training_steps=train_iter)
        ...
        iter_right_dis = 0.0
        iter_sample = 0.0
        # scaler = GradScaler(init_scale=65536.0 / 128.0)
        ...
        with torch.autograd.set_detect_anomaly(True):
            for it in range(start_iter, start_iter + train_iter):
                ...
                model.first_update(optimizer_G)
                # Loss_scond = model.sconde_update(logit_old)
                loss = Loss + Loss1
                loss /= float(grad_iter)
                right = [model.accuracy(pred[0], label), model.accuracy(pred[1], label)]

                loss.backward()
                # print("\n\n\n")
                # output_model_put(model)
                # pdb.set_trace()
                if it % grad_iter == 0:
                ...
                predict, Loss_first = model.sconde_update(step=it)
                right.append(model.accuracy(predict[0], label))
                Loss_first.backward()
                if it % grad_iter == 0:
                    torch.nn.utils.clip_grad_norm_(model.parameters(), 10.0)
                    optimizer_G.step()
                ...
                if (it + 1) % 100 == 0:
                    time_end = time.time()
                    sys.stdout.write(
                    'step: {0:4} | loss: {1:2.6f}, loss1: {2:2.6f}, accuracy: {3:3.2f}%, Time:{4:3.4f}'.format(
                            it + 1, iter_loss / iter_sample, iter_loss1 / iter_sample,
                            100 * iter_right[0] / iter_sample, (time_end - time_start) / 60.0) + '\r')
                    sys.stdout.flush()
                    time_start = time.time()
    ...
    def eval(self,
        ...
        with torch.no_grad():
            for it in range(eval_iter):
                ...
                # predict, _ = model.get_global_loss(retain=True)
                predict, _ = model.get_Poto_loss()

                right = [model.accuracy(pred[0], label),model.accuracy(pred[1], label),model.accuracy(predict, label)]
                ...
                if (it + 1) % 100 == 0:
                    ...
                    it + 1, 100 * iter_right[0] / iter_sample) + '\r')
                    sys.stdout.flush()
            print("")
